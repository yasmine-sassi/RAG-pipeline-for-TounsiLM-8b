...
% Inference flow
\node[box]    (query)   {User query\\(Arabic / Arabizi / French)};
\node[newbox, right=of query]   (rewriter) {Query Rewriter\\+ Type Router};
\node[newbox, right=of rewriter](hybrid)   {Hybrid Retrieval\\BM25 + Semantic};
\node[store,  below=of hybrid]  (chroma)   {ChromaDB\\(vectors)};
\node[box,    left=of chroma]   (merge)    {Multi-query merge\\(best score)};
\node[box,    left=of merge]    (context)  {Context\\formatting};
\node[box,    below=of context] (llm)      {TounsiLM-8b\\(generation)};
\node[box,    below=of llm]     (answer)   {Answer +\\confidence level};

% Arrows — inference
\draw[arrow] (query)    -- (rewriter);
\draw[arrow] (rewriter) -- node[above,label]{variants} (hybrid);
...
% Index path (dashed)
\node[box, above=1.3cm of chroma, fill=green!8] (indexer) {Indexing\\(\texttt{passage:})};
\node[box, right=of indexer, fill=green!8]       (kb)      {Knowledge base\\(JSON --- 1{,}647 entries)};
\draw[arrow, dashed] (kb)      -- (indexer);
\draw[arrow, dashed] (indexer) -- node[right,label]{index} (chroma);

\end{tikzpicture}
\caption{RAG pipeline architecture. Purple boxes are newly introduced components.
The dashed path is the one-time indexing phase; the solid path is the inference flow.}
